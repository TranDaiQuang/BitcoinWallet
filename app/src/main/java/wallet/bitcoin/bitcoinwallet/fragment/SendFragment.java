...
import wallet.bitcoin.bitcoinwallet.dialog.BuyBitcoinDlgHelper;
...
public class SendFragment extends BaseFragment implements SendAdapter.OnSendAdapterCallback, BuyBitcoinDlgHelper.BuyBitcoinDlgCallback {
    ...
    private BuyBitcoinDlgHelper buyBitcoinDlgHelper;
    ...
    private void showTipsDlg() {
        if (getActivity() != null && ((MainActivity) getActivity()).getBalance() == 0) {
            buyBitcoinDlgHelper = BuyBitcoinDlgHelper.show(getActivity(), this);
        }
    }
    ...
    public void onSendBtnClicked(boolean isFinalConfirmation) {
        if (getActivity() != null) {
            UIHelper.hideKeyboard(getActivity());
            ...
            if (isFinalConfirmation) {
                ...
                if (sendingAddress == null) {
                    ...
                    sendMoneyFromAddress(sendingAddress);
                }
            } else {
                ((MainActivity) getActivity()).setFeeVisibility(Fee.OPTIMAL_FEE);
            }
        }
    }
    ...
    private void sendMoneyFromWallet() {
        showProgress(true);
        ...
        if (getActivity() != null) {
            ((MainActivity) getActivity()).setFeeInVisibility();
        }
    }

    private void sendMoneyFromAddress(String sendingAddress) {
        showProgress(true);
